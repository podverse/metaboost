...
2. **Prop API** — design `@metaboost/ui` surface with **no embedded user-facing copy** (see **shared-ui-i18n**).
...
5. **App migration** — keep wrappers thin where required (`next/link`, `next-intl`, session guards). The same “thin wrapper” approach applies **inside one app** when identical configured usage repeats (see **reusable-components**).
...
- **shared-ui-i18n** — apps localize; `@metaboost/ui` stays copy-free.
- **reusable-components** — default preference order and anti-patterns.
- **modal-layout-contract** — shared dialog shells (`ModalDialogContent`, `ConfirmDeleteModal`).
