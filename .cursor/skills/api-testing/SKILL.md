...
- **Signup strict-limit test mode**: For `POST /auth/signup` strict-limit assertions, force signup-enabled mode in the test setup (`MAILER_ENABLED=true` and `ACCOUNT_SIGNUP_MODE` not `admin_only`) so assertions are not masked by admin-only/no-mailer `403` responses. Keep this setup before the dynamic app import.
...
3. **New env mode (e.g. ACCOUNT_SIGNUP_MODE or new feature flag)**  
