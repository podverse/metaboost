...
- **Requirements**: Before tests, Postgres and Valkey must be up and the test DB created. Root `npm run test` runs `scripts/check-test-requirements.mjs` first; if ports are unreachable, it exits with instructions. From repo root: `make test_deps` (note underscore) starts containers (ports 5532, 6479), creates `metaboost_app_test`, applies schema, and grants read/read_write (including TRUNCATE for globalSetup). Make targets use **underscores**: `test_deps`, `help_test`, `test_clean`.
...
- Test env: `apps/api/src/test/setup.ts` sets defaults (DB_PORT 5532, VALKEY_PORT 6479, DB_APP_NAME metaboost_app_test, etc.). globalSetup uses the same defaults so it can run without setupFiles.
