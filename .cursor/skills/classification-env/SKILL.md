...
- **Keep** **`render_k8s_env.rb`** bucket logic aligned with **`kind`** (buckets use **`BoilerplateEnvMerge.effective_env_group_var_specs`** via **`derive_render_buckets`**).

## Don't

- Do not reintroduce **`apps/*/.env.example`** or **`dev/env-overrides/examples/`** as the source of truth; use **`infra/env/classification/base.yaml`** + optional home overrides (`~/.config/boilerplate/`) and this skill.

## Related

- **argocd-gitops-push** — When changing K8s manifests or GitOps outputs, remind to push the GitOps repo.
- **`docs/development/REMOTE-K8S-GITOPS.md`** — Generic remote cluster + GitOps flow (no vendor domains).
