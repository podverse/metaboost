...
Use `SPEC` paths relative to the app `e2e/` folder (e.g. `e2e/login.spec.ts`), not `apps/web/e2e/...`.
...
- API-only, ORM, or config-only changes with no UI impact: follow **response-ending-make-verify**.
- Documentation-only edits under `docs/` with no component changes.

## Related

- **response-ending-make-verify** — command tree and API gate policy
- **end-with-targeted-make-report-verify** rule — mandatory E2E block when UI or E2E specs change
