...
- `.cursor/skills/build-order/SKILL.md` (when adopted)
- [AGENTS.md](/AGENTS.md) § Package Build Order

Do not merge build-order changes that leave contributor build docs stale.

## Theme fixture + docs sync guardrail

When theme CSS variables are added, removed, or renamed (for example in `packages/ui/src/styles/_themes.scss`
and related token files), keep theme examples and docs in lockstep.

Required follow-ups in the same change:

- Update maintained custom-theme fixture JSON under `tools/test-assets/assets/themes/` when present.
- Update docs that reference those fixtures or remote custom-theme setup (`docs/development/env/ENV-REFERENCE.md`, `docs/STYLES-SCSS.md` when applicable).

Do not merge theme-token changes until fixture paths are created and documented as canonical operator/E2E examples.
