...
export function createMbrssV1Router(): Router {
  ...
  router.get('/boost/:bucketShortId', asyncHandler(mbrssV1Controller.getBoostCapability));
  router.post(
    '/boost/:bucketShortId',
    validateBody(createMbrssV1BoostSchema),
    asyncHandler(mbrssV1Controller.createBoostMessage)
  );
  router.get('/messages/public/:bucketShortId', asyncHandler(mbrssV1Controller.listPublicMessages));
  router.get(
