...
describe('bucket roles', () => {
  ...
  describe('GET /buckets/:bucketId/roles', () => {
    it('returns 401 when unauthenticated', async () => {
      await request(app).get(`${API}/buckets/${rootBucketShortId}/roles`).expect(401);
    });
    ...
    it('returns 200 with roles array including predefined roles when authenticated', async () => {
      ...
      const res = await agent.get(`${API}/buckets/${rootBucketShortId}/roles`).expect(200);
  ...
  describe('POST /buckets/:bucketId/roles', () => {
    it('returns 401 when unauthenticated', async () => {
      await request(app)
        .post(`${API}/buckets/${rootBucketShortId}/roles`)
    ...
    it('returns 400 when creating role on non-root bucket (requireRoot enforced)', async () => {
      const rootBucket = await BucketService.findByShortId(rootBucketShortId);
      ...
      const res = await agent
        .post(`${API}/buckets/${mid.shortId}/roles`)
  ...
  describe('PATCH /buckets/:bucketId/roles/:roleId', () => {
    it('returns 404 for nonexistent roleId', async () => {
      ...
      await agent
        .patch(`${API}/buckets/${rootBucketShortId}/roles/00000000-0000-4000-a000-000000000000`)
        .send({ name: 'Nope' })
    ...
    it('returns 200 with updated role when body valid', async () => {
      ...
      const res = await agent
        .patch(`${API}/buckets/${rootBucketShortId}/roles/${roleId}`)
  ...
  describe('DELETE /buckets/:bucketId/roles/:roleId', () => {
    it('returns 404 for nonexistent roleId', async () => {
      ...
      await agent
        .delete(`${API}/buckets/${rootBucketShortId}/roles/00000000-0000-4000-a000-000000000000`)
        .expect(404, { message: 'Role not found' });
    });
    ...
    it('returns 204 when role exists and is deleted', async () => {
      ...
      await agent.delete(`${API}/buckets/${rootBucketShortId}/roles/${roleId}`).expect(204);
    });
    ...
    it('role no longer appears in list after delete', async () => {
      ...
      const listRes = await agent.get(`${API}/buckets/${rootBucketShortId}/roles`).expect(200);
