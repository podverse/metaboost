...
test.describe('Invite-page for the bucket-admin user', () => {
  test('When the non-owner-admin opens the invite-page with an invalid token, they see the invalid state.', async ({
    page,
  }, testInfo) => {
    ...
    await capturePageLoad(
      page,
      testInfo,
      'The invite-page shows invalid state for the non-owner-admin with an invalid token.'
    );
  });

  test('When the non-owner-admin opens the invite-page with a valid token, they see accept or reject actions.', async ({
    page,
  }, testInfo) => {
    setE2EUserContext(testInfo, 'bucket-admin');
    ...
    await actionAndCapture(
      page,
      testInfo,
      'User opens a valid invite while authenticated as admin and sees accept or reject actions.',
      async () => {
    ...
    await capturePageLoad(
      page,
      testInfo,
      'The invite-page shows accept/reject or final state for the non-owner-admin.'
    );
  });
});
