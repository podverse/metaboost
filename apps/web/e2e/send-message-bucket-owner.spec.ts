...
test.describe('Public send-message-page for the bucket-owner user', () => {
  test('When the authenticated owner opens the public send-message-page for a public bucket, they see the send-message form.', async ({
    page,
  }, testInfo) => {
    ...
    await capturePageLoad(
      page,
      testInfo,
      'The public send-message-page shows the form for the authenticated owner.'
    );
  });

  test('When the owner opens the send-message-page with an invalid short id, they see not found.', async ({
    page,
  }, testInfo) => {
    setE2EUserContext(testInfo, 'bucket-owner');
    await loginAsWebE2EUserAndExpectDashboard(page);
  ...
  test('When the owner opens the send-message-page for a private bucket, they see not found.', async ({
    page,
  }, testInfo) => {
    setE2EUserContext(testInfo, 'bucket-owner');
    await loginAsWebE2EUserAndExpectDashboard(page);
