...
test.describe('User-settings-page for the bucket-owner user (admin_only_email)', () => {
  ...
  test('When the user submits a new email and email verification is enabled, they see the success message that a verification email was sent.', async ({
    page,
  }, testInfo) => {
    ...
    const newEmail = `e2e-emailchange-${Date.now()}@example.com`;
    await newEmailInput.fill(newEmail);

    await actionAndCapture(
      page,
      testInfo,
      'User submits request-email-change and sees success message that verification email was sent.',
      async () => {
        await page.getByRole('button', { name: /request email change|send|submit/i }).click();
        await expect(
          page.getByText(/verification email sent|email sent to the new address/i).first()
        ).toBeVisible();
        await expect(page).toHaveURL(/\/settings\?tab=email/);
      }
    );
