...
#### Prompt (Developer)
for metaboost the default targetRevision for base k8s files should be staging. the "alpha" k8s files should be X.Y.Z-staging.N

#### Key Decisions
- Set local/base Argo Application defaults to `targetRevision: staging`.
- Set all alpha app-of-apps and child Application `targetRevision` values to `X.Y.Z-staging.N`.
- Align alpha remote Kustomize base `?ref=` pins to `X.Y.Z-staging.N` for revision consistency.
...
#### Verification
- `rg -n "targetRevision:|\?ref=" infra/k8s/alpha infra/k8s/local-application.yaml infra/k8s/argocd/metaboost-local-stack-application.yaml infra/k8s/INFRA-K8S.md` shows local/base `targetRevision: staging` and alpha `targetRevision`/`?ref=` values set to `X.Y.Z-staging.N`.
