...
check metaboost and metaboost.cc for any examples where a "latest" tag is used. we want everything hard coded to X.Y.Z-staging.N for now
...
- Kept **`infra/k8s/base/stack/workloads.yaml`** on explicit `metaboost-local-*:X.Y.Z-staging.N` (no `:latest` in that K8s manifest).
- **`build-images.sh`:** dual-tag each `metaboost-local-*` image as `:latest` and `:${METABOOST_LOCAL_K8S_IMAGE_TAG:-X.Y.Z-staging.N}` so Compose-oriented `latest` and K8s pins refer to the same build.
