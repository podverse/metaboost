...
- Prefer **named exports**; avoid `export default` in ordinary modules when a named export works. See **.llm/exports/opencode/skills/prefer-named-exports/SKILL.md** (Next.js `page` defaults excepted).
...
- Documentation naming rules are canonical in **.llm/exports/opencode/skills/documentation-conventions/SKILL.md**.

## LLM History

- Follow **.llm/exports/opencode/skills/llm-history/SKILL.md** as the canonical history process (timing, format, and split rules).
- See `.llm/LLM.md` for the full system documentation.

## Code Quality

- **Import path casing:** Relative import paths must match the exact casing of files and directories on disk so builds pass on Linux/CI. See **.llm/exports/opencode/skills/path-casing-imports/SKILL.md** and **.llm/exports/opencode/instructions/path-casing-imports.instructions.md**.
...
- **Catch blocks:** If the error value is not used, use `catch { ... }` (no variable). Put at least one comment inside the block when swallowing errors so ESLint **`no-empty`** passes. See **.llm/exports/opencode/skills/catch-unused-error/SKILL.md**.
- **Unused props/vars:** Remove them; do not keep and destructure as `_unused`. See **.llm/exports/opencode/skills/avoid-unused-props-vars/SKILL.md**.
