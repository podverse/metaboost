#!/usr/bin/env bash
# Helper to create an encrypted metaboost API secret manifest.
...
SECRET_NAME="metaboost-api-secrets"
NAMESPACE="metaboost-${ENVIRONMENT}"
OUTPUT_FILE="./secrets/metaboost-${ENVIRONMENT}-api-secrets.enc.yaml"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/metaboost-api-secret.XXXXXX")"
TMP_FILE="${TMP_DIR}/metaboost-api-secrets.yaml"
