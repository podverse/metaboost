#!/usr/bin/env bash
# Ops-only: mirrors StatefulSet bootstrap `0001_create_app_db_users.sh` +
# `0002_setup_management_database.sh` over TCP with passwords (PGHOST/PGPASSWORD).
# Keep aligned when editing:
...
#
# Required env: metaboost-db-secrets keys (same as bootstrap shells).
...
echo "rebootstrap-full-bootstrap: Phase B — management roles + DB same as 0002_setup_management_database.sh"
