...
| **G3 — Metaboost verification readiness** | Standard Endpoint `POST` requests require a valid AppAssertion where enforcement is enabled; verifier, registry poll, replay store, and HTTPS policy behave per [ENV-REFERENCE.md](/docs/development/env/ENV-REFERENCE.md).                                                                                      | Integration tests green; staging smoke (section 3) passes.                                         |
...
| **Local dev**                                  | HTTPS requirement may be relaxed per env template; behavior matches [ENV-REFERENCE.md](/docs/development/env/ENV-REFERENCE.md).                                                                                  |
