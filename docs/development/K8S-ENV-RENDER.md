...
- **`kind: literal`** — Non-secret; emitted into **`source/boilerplate-<suffix>-config.env`** (single dotenv file per workload) by `render_k8s_env.rb` when present in merged env (same as **`kind: config`** for K8s). The GitOps overlay **`kustomization.yaml`** uses **`configMapGenerator`** with **`envs:`** pointing at that file (Podverse-style). Values are escaped for Kustomize’s dotenv parser; **`make alpha_env_validate`** runs **`kubectl kustomize`** on each overlay to catch bad lines. Local `.env` generation may still treat literals separately from **`kind: config`**.
- **`kind: config`** — Emitted into the same **`source/boilerplate-<suffix>-config.env`** when present in merged env (then into a ConfigMap via the overlay **`configMapGenerator`**).
...
**`source/boilerplate-*-config.env`** files consumed by **`configMapGenerator`** **`envs:`** in the same **`kustomization.yaml`**, **`deployment-secret-env.yaml`**, generated
...
- Add **`generatorOptions.disableNameSuffixHash: true`** and **`configMapGenerator`** with **`envs: [source/boilerplate-<suffix>-config.env]`** in the component **`kustomization.yaml`** (do **not** list the dotenv under **`resources:`**), and add **`patchesStrategicMerge: [deployment-secret-env.yaml]`** in the same **`kustomization.yaml`** where secrets exist.
...
The **`db`** env group lists **all** database-related keys under **`env_groups.db.vars`** in classification. Local dev writes them into one **`infra/config/local/db.env`** via **`merge-env --profile local_docker --group db`**. Postgres **role names and passwords** are **secret**; **`DB_HOST_SOURCE_ONLY`** / **`DB_PORT_SOURCE_ONLY`** are **`source_only`** (omitted from pod env by render); **`DB_HOST`** / **`DB_PORT`** are **`literal`** and appear in **`source/boilerplate-db-config.env`** when merged (same as other non-secret literals), then in-cluster as a ConfigMap.
...
- `make alpha_env_render` — Emit **`source/boilerplate-*-config.env`** per workload, **`deployment-secret-env.yaml`** patches, port + ingress patches (see **Port sync** above), and cleartext Secrets under `secrets/boilerplate-<env>/plain/` (requires `BOILERPLATE_K8S_OUTPUT_REPO`). Prunes generator-owned paths first (see above).
...
- `make alpha_env_kustomize_check` — [`validate-gitops-kustomize-build.sh`](../../scripts/k8s-env/validate-gitops-kustomize-build.sh): runs **`kubectl kustomize`** on each component overlay (requires `BOILERPLATE_K8S_OUTPUT_REPO`).
- `make alpha_env_validate` — [`validate-classification.sh`](../../scripts/k8s-env/validate-classification.sh) + [`validate-k8s-env-drift.sh`](../../scripts/k8s-env/validate-k8s-env-drift.sh) + **`alpha_env_kustomize_check`** (requires `BOILERPLATE_K8S_OUTPUT_REPO`).
...
2. **Config env and patch drift** — [`validate-k8s-env-drift.sh`](../../scripts/k8s-env/validate-k8s-env-drift.sh) renders into a
   temp directory with the same inputs as `make alpha_env_render`, then byte-compares each generated **`source/boilerplate-*-config.env`**, each
...
2. Commit **`source/boilerplate-*-config.env`** under `apps/boilerplate-<env>/*/`, generated **`deployment-secret-env.yaml`**, **`deployment-ports-and-probes.yaml`**, and **`common/ingress-port-backends.yaml`**.
