...
For the full classified variable list with expected defaults and where each key appears in Kubernetes, see [ENV-VARS-CATALOG.md](ENV-VARS-CATALOG.md).
...
- [K8S-ENV-RENDER.md](k8s/K8S-ENV-RENDER.md) – Render ConfigMaps and Secrets for alpha/beta/prod K8s from the same `.env.example` contract (`make alpha_env_render`, `dev/env-overrides/<env>/`).
