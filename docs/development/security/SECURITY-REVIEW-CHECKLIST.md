...
- **JWT**: Signing and verification must use the same per-service secret (**`AUTH_JWT_SECRET`** in that service’s env / Kubernetes Secret) and preserve expiration handling; middleware must continue validating both `sub` and `id_text` against loaded users.
...
From repo root (after [`docs/CURSOR-NIX-WITH-ENV.md`](/docs/CURSOR-NIX-WITH-ENV.md) if needed):
