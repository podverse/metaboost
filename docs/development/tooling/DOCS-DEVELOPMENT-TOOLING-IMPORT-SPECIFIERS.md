...
**Where:** `packages/**` **except** `packages/ui/**`, plus `apps/api`, `apps/management-api`, `apps/*/sidecar`, `tools/**`, and `scripts/**/*.ts` / `*.mts` (see ESLint config).
...
**Where:** `packages/ui/**` (published `@metaboost/ui`, transpiled by consuming Next apps).
...
**Where:** `apps/web/src/**`, `apps/management-web/src/**`, and the same apps’ Playwright `e2e/**`.

**Rule:** Keep **extensionless** relative imports unless you have verified **`next build`** with Turbopack for your change. Do **not** bulk-codemod Tier B to `.js` specifiers yet.

**Why:** Turbopack’s resolution for `.js` specifiers pointing at `.tsx`/`.ts` sources does not yet match the webpack extension-alias workflow many codebases used. Upstream tracking: [vercel/next.js#82945](https://github.com/vercel/next.js/issues/82945).
