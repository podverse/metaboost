...
When a PR merges to `develop`, `.github/workflows/complete-feature.yml` may archive matching
`.llm/history/active/<feature-name>/` folders automatically (see [.llm/LLM.md](/.llm/LLM.md)).
...
This skill provided guidelines for maintaining LLM development history in the Metaboost repo. The Cursor rule `llm-history-tracking.mdc` was the always-applied requirement.
