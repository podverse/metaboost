...
- **`kind: source_only`** — In merged env for this env group but not emitted into that group’s CM/Secret.

`scripts/k8s-env/render-k8s-env.sh` builds a merged env per classification env group with profile **`remote_k8s`**, an optional **GitOps classification overlay**, then `dev/env-overrides/<env>/*.env` (see [ENV-REFERENCE.md](../env/ENV-REFERENCE.md)).
...
- Keep **`envFrom.configMapRef`** where the overlay app has non-secret config keys (the **`db`** app may have none after **`DB_APP_NAME`** is wired from the Secret in **`deployment-postgres.yaml`**).
...
**Postgres:** The official image expects **`DB_APP_ADMIN_USER`** / **`DB_APP_ADMIN_PASSWORD`**; keep those as **hand-maintained** `env` entries in **`deployment-postgres.yaml`** mapping to **`DB_APP_ADMIN_USER`** / **`DB_APP_ADMIN_PASSWORD`** in **`metaboost-db-secrets`**. The generated patch still adds **`DB_*`** keys the init scripts need. **`Valkey`** uses **`VALKEY_PASSWORD`** in the generated patch; non-secret **`VALKEY_HOST`** / **`VALKEY_PORT`** (when present) render into **`source/metaboost-valkey-config.env`** and a generated ConfigMap.
