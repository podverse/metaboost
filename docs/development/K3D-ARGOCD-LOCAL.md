...
7. **Bootstrap ArgoCD** – applies `infra/k8s/argocd-project.yaml`, `infra/k8s/local-application.yaml` (parent app), and `infra/k8s/argocd/boilerplate-local-stack-application.yaml` (stack Application CR from disk).
8. **Apply local stack** – `kubectl apply -k infra/k8s/local/stack` from your **working tree**. The stack Application uses **manual** sync only, and the parent app no longer auto-syncs with self-heal, so this is not reset to GitHub on every reconcile. Use **Sync** on `boilerplate-local-stack` in Argo CD when you want the cluster to match the Git revision in that Application.
...
- `infra/k8s/argocd/boilerplate-local-stack-application.yaml` sets `targetRevision` for the stack (branch Argo CD uses when you **Sync** that app). Align with your workflow (e.g. `feature/k8s-alpha`, `main`, or `develop`).
