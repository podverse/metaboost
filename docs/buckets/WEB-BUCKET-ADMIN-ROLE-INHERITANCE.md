...
- `minimumMessageAmountMinor` is edited at the top-level bucket only and defaults to `0` in root preferred-currency minor units.
- Message list endpoints (owner/admin/public standard) apply an effective minimum of `max(request minimumAmountMinor, root minimumMessageAmountMinor)` using create-time threshold snapshots.
