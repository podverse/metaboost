...
- `publicBoostDisplayMinimumMinor` (optional public list floor) is edited at the top-level bucket only and defaults to `0` in root preferred-currency minor units.
- Message list endpoints (owner/admin/public standard) apply an effective minimum of `max(request minimumAmountMinor ?? 0, root publicBoostDisplayMinimumMinor)` using create-time threshold snapshots.
