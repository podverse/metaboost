...
| Indexes                 | infra/database                          | Indexes exist on `owner_id`, `parent_bucket_id`, `id_text`, `bucket_admin(bucket_id, user_id)`, `bucket_message(bucket_id, created_at)`, etc. Add indexes only if profiling shows slow queries. | Done (indexes present) |
