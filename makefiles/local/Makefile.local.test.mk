...
.PHONY: sync_k8s_postgres_init check_k8s_postgres_init_sync

# Canonical DB source is infra/k8s/base/db/source (no copy step required).
...
TEST_APP_MIGRATOR_USER ?= metaboost_app_migrator
TEST_APP_MIGRATOR_PASSWORD ?= test
TEST_APP_READ_USER ?= metaboost_app_read
TEST_APP_READ_PASSWORD ?= test
TEST_APP_READ_WRITE_USER ?= metaboost_app_read_write
TEST_APP_READ_WRITE_PASSWORD ?= test
TEST_MGMT_MIGRATOR_USER ?= metaboost_management_migrator
TEST_MGMT_MIGRATOR_PASSWORD ?= test
TEST_MGMT_READ_USER ?= metaboost_management_read
TEST_MGMT_READ_PASSWORD ?= test
TEST_MGMT_READ_WRITE_USER ?= metaboost_management_read_write
...
# Create metaboost_app_test database, apply linear migration chain, create app DB users and grants.
...
test_db_init: test_postgres_up
	...
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d postgres -c "DO \$$$$ BEGIN IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '$(TEST_APP_MIGRATOR_USER)') THEN CREATE USER $(TEST_APP_MIGRATOR_USER) WITH PASSWORD '$(TEST_APP_MIGRATOR_PASSWORD)'; ELSE ALTER USER $(TEST_APP_MIGRATOR_USER) WITH PASSWORD '$(TEST_APP_MIGRATOR_PASSWORD)'; END IF; IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '$(TEST_APP_READ_USER)') THEN CREATE USER $(TEST_APP_READ_USER) WITH PASSWORD '$(TEST_APP_READ_PASSWORD)'; END IF; IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '$(TEST_APP_READ_WRITE_USER)') THEN CREATE USER $(TEST_APP_READ_WRITE_USER) WITH PASSWORD '$(TEST_APP_READ_WRITE_PASSWORD)'; END IF; END \$$$$;"
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_DB_NAME) -c "ALTER DATABASE $(TEST_DB_NAME) OWNER TO $(TEST_APP_MIGRATOR_USER);"
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_DB_NAME) -c "GRANT CONNECT ON DATABASE $(TEST_DB_NAME) TO $(TEST_APP_MIGRATOR_USER), $(TEST_APP_READ_USER), $(TEST_APP_READ_WRITE_USER);"
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_DB_NAME) -c "GRANT USAGE, CREATE ON SCHEMA public TO $(TEST_APP_MIGRATOR_USER);"
	@METABOOST_LOCAL_PG_CONTAINER="$(TEST_PG_CONTAINER)" DB_HOST="127.0.0.1" DB_PORT="$(TEST_DB_PORT)" DB_APP_NAME="$(TEST_DB_NAME)" DB_APP_MIGRATOR_USER="$(TEST_APP_MIGRATOR_USER)" DB_APP_MIGRATOR_PASSWORD="$(TEST_APP_MIGRATOR_PASSWORD)" \
	bash scripts/database/run-linear-migrations.sh --database app
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_DB_NAME) -c "REASSIGN OWNED BY $(TEST_APP_MIGRATOR_USER) TO $(TEST_APP_READ_WRITE_USER);"
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_DB_NAME) -c " \
		...
		ALTER DEFAULT PRIVILEGES FOR ROLE $(TEST_APP_MIGRATOR_USER) IN SCHEMA public GRANT SELECT ON TABLES TO $(TEST_APP_READ_USER); \
		ALTER DEFAULT PRIVILEGES FOR ROLE $(TEST_APP_MIGRATOR_USER) IN SCHEMA public GRANT SELECT ON SEQUENCES TO $(TEST_APP_READ_USER); \
		ALTER DEFAULT PRIVILEGES FOR ROLE $(TEST_APP_MIGRATOR_USER) IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON TABLES TO $(TEST_APP_READ_WRITE_USER); \
		ALTER DEFAULT PRIVILEGES FOR ROLE $(TEST_APP_MIGRATOR_USER) IN SCHEMA public GRANT SELECT, USAGE, UPDATE ON SEQUENCES TO $(TEST_APP_READ_WRITE_USER);"
	@echo "Test database $(TEST_DB_NAME) and users ready."

# Create metaboost_management_test database and apply management linear migrations. Requires test_db_init (app users).
...
test_db_init_management: test_db_init
	...
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d postgres -c "DO \$$$$ BEGIN IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '$(TEST_MGMT_MIGRATOR_USER)') THEN CREATE USER $(TEST_MGMT_MIGRATOR_USER) WITH PASSWORD '$(TEST_MGMT_MIGRATOR_PASSWORD)'; ELSE ALTER USER $(TEST_MGMT_MIGRATOR_USER) WITH PASSWORD '$(TEST_MGMT_MIGRATOR_PASSWORD)'; END IF; IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '$(TEST_MGMT_READ_USER)') THEN CREATE USER $(TEST_MGMT_READ_USER) WITH PASSWORD '$(TEST_MGMT_READ_PASSWORD)'; END IF; IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = '$(TEST_MGMT_READ_WRITE_USER)') THEN CREATE USER $(TEST_MGMT_READ_WRITE_USER) WITH PASSWORD '$(TEST_MGMT_READ_WRITE_PASSWORD)'; END IF; END \$$$$;"
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_MANAGEMENT_DB_NAME) -c "ALTER DATABASE $(TEST_MANAGEMENT_DB_NAME) OWNER TO $(TEST_MGMT_MIGRATOR_USER);"
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_MANAGEMENT_DB_NAME) -c "GRANT CONNECT ON DATABASE $(TEST_MANAGEMENT_DB_NAME) TO $(TEST_MGMT_MIGRATOR_USER), $(TEST_MGMT_READ_USER), $(TEST_MGMT_READ_WRITE_USER);"
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_MANAGEMENT_DB_NAME) -c "GRANT USAGE, CREATE ON SCHEMA public TO $(TEST_MGMT_MIGRATOR_USER);"
	@METABOOST_LOCAL_PG_CONTAINER="$(TEST_PG_CONTAINER)" DB_HOST="127.0.0.1" DB_PORT="$(TEST_DB_PORT)" DB_MANAGEMENT_NAME="$(TEST_MANAGEMENT_DB_NAME)" DB_MANAGEMENT_MIGRATOR_USER="$(TEST_MGMT_MIGRATOR_USER)" DB_MANAGEMENT_MIGRATOR_PASSWORD="$(TEST_MGMT_MIGRATOR_PASSWORD)" \
	...
	@docker exec $(TEST_PG_CONTAINER) psql -U $(TEST_PG_USER) -d $(TEST_MANAGEMENT_DB_NAME) -c " \
		...
		ALTER DEFAULT PRIVILEGES FOR ROLE $(TEST_MGMT_MIGRATOR_USER) IN SCHEMA public GRANT SELECT ON TABLES TO $(TEST_MGMT_READ_USER); \
		ALTER DEFAULT PRIVILEGES FOR ROLE $(TEST_MGMT_MIGRATOR_USER) IN SCHEMA public GRANT SELECT ON SEQUENCES TO $(TEST_MGMT_READ_USER); \
		ALTER DEFAULT PRIVILEGES FOR ROLE $(TEST_MGMT_MIGRATOR_USER) IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON TABLES TO $(TEST_MGMT_READ_WRITE_USER); \
		ALTER DEFAULT PRIVILEGES FOR ROLE $(TEST_MGMT_MIGRATOR_USER) IN SCHEMA public GRANT SELECT, USAGE, UPDATE ON SEQUENCES TO $(TEST_MGMT_READ_WRITE_USER);"
...
help_test:
	@echo "Test requirements: Postgres on port $(TEST_DB_PORT) and Valkey on port $(TEST_KEYVALDB_PORT), with databases $(TEST_DB_NAME) and $(TEST_MANAGEMENT_DB_NAME), and DB users $(TEST_APP_READ_USER), $(TEST_APP_READ_WRITE_USER), $(TEST_MGMT_READ_USER), $(TEST_MGMT_READ_WRITE_USER)."
	@echo ""
	...
	@echo "  3. Drop and recreate $(TEST_DB_NAME), run linear migrations (--database app), and create $(TEST_APP_READ_USER)/$(TEST_APP_READ_WRITE_USER) users."
	@echo "  4. Drop and recreate $(TEST_MANAGEMENT_DB_NAME), run linear migrations (--database management) (for management-api tests)."
