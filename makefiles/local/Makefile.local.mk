...
#   local_clean               - Run local_down, local_down_volumes, local_k3d_down, and test_clean (full teardown: Docker + k3d + test/E2E; also removes orphan boilerplate_local_* containers)
#   local_env_prepare          - Ensure ~/.config/boilerplate/local-env-overrides/ exists (optional overrides; defaults in infra/env/classification)
...
#   alpha_env_prepare, alpha_env_link, alpha_env_clean, alpha_env_prepare_link, alpha_env_render, alpha_env_render_dry_run, alpha_env_validate - GitOps K8s env (see docs/development/K8S-ENV-RENDER.md; makefiles/gitops/Makefile.gitops-env.mk); BOILERPLATE_K8S_OUTPUT_REPO required for render/validate; k8s_env_* + K8S_ENV for beta/prod (k8s_env_clean removes dev/env-overrides/$(K8S_ENV) symlinks)
#
SHELL := /bin/bash
