/**
 * i18n compile: sync overrides from originals, merge originals + overrides into compiled.
 * Run from repo root: node scripts/i18n/compile.mjs <appName>
 * Example: node scripts/i18n/compile.mjs web
 */
...
if (!appName) {
  console.error('Usage: node scripts/i18n/compile.mjs <appName> (e.g. web or management-web)');
  process.exit(1);
}
...
const compiledDir = path.join(appDir, 'i18n', 'compiled');
...
for (const locale of localeFiles) {
  ...
  if (locale === 'en-US') {
    ...
    console.warn('[' + appName + '] Compiled ' + locale + '.json (source)');
  } else {
    ...
    console.warn('[' + appName + '] Compiled ' + locale + '.json');
  }
}

console.warn('[' + appName + '] i18n compile complete');
