...
Shared utilities for Boilerplate: startup env validation and DB field-length constants.
...
- **DB field lengths** – `EMAIL_MAX_LENGTH`, `PASSWORD_HASH_LENGTH`, `SHORT_TEXT_MAX_LENGTH` align with `infra/database/migrations` (e.g. varchar_email, varchar_password, varchar_short). Use in ORM entities and app validation so values stay in sync.
